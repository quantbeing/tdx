...
| Fund flow | yes | Today flow is derived from paged transaction records with xmtdx amount thresholds. `GetFundFlowWithOptions` exposes page-size/start/page-count budgets and returns an explicit page-budget error instead of silently truncating. |
