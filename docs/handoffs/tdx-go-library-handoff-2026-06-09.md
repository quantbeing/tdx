...
- 2026-06-11 CLI retry controls added `-max-attempts`, `-retry-strategy`, and `-same-host-attempts` to `tdx-validate`, `tdx-probe`, and `tdx-health -probe`. `tdx-op-matrix` remains `MaxAttempts=1` by design so per-host evidence is not mixed by failover.
