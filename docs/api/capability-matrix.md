...
| Security count/list | yes | Raw/unknown fields preserved. `ListSecuritiesWithOptions` and `ListASharesWithOptions` add page-budget controls with typed partial failures; callers can classify partial-result errors with `IsPartialResultError`. `tdx-validate -full-security-list` pages through the complete selected-market list, emits per-page results, checks count parity plus duplicate symbols, and supports page-level retry through `-security-list-page-retries`. |
...
| CLI diagnostics | yes | `tdx-health`, `tdx-probe`, `tdx-data-probe`, `tdx-op-matrix`, `tdx-fixture-matrix`, `tdx-validate`, `tdx-dump-frame`, and `tdx-compare-py` are implemented; `tdx-health -probe` selects a host by operation probes, while matrix output can be JSON or JSONL and continues after per-operation failures. |
