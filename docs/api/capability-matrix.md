...
| Block/report file | partial | Metadata, chunk fetch, report fetch loop, and local `.dat` board parser implemented. Empty report payloads, empty block metadata, and invalid/empty block payloads now return explicit errors. 2026-06-09 live `boards_concept` returned rows, but `base_info.zip` returned an empty public-server payload. |
